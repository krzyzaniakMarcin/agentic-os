...
> **Every append takes a transaction-level advisory lock (`pg_advisory_xact_lock`) around the insert, in `log.py`'s emit.** That serializes commits across *any* number of writer processes and connections — and there are several: each stateless `claude -p` step spawns its own stdio MCP server process, and the harness writes `agent.step` through `log.py` directly, so a "single writer connection" can't be a deployment constraint. At this throughput the lock is free. (Alternative if it ever bites: readers bounding by `pg_snapshot_xmin(pg_current_snapshot())`.)
...
- `substrate/log.py` + `substrate/mcp_server.py`: Postgres event log with `emit_event` / `read_events`, **serialized appends (monotonic visibility, §4)**, **self-exclusion**, and **server-side identity**.

...
### T14 — `topologies/supervisor.yaml` + run-config loader ✅ done
- **Landed:** `kernel/config.py` (`load_run_config`, adds a `pyyaml` dep — no YAML parser in stdlib) maps the YAML `rails.timeout_s` → the cfg key `run_timeout_s` `run_episode` reads; `topologies/supervisor.yaml` carries the supervisor + worker roles/prompts; `orchestrator.main(topology_path=None)` loads it (path via argv/env, else default) and `_stub_cfg` is retired. `runtime`/`model` role fields are inert in Phase 1 (dropped by `load_role`) — no runtime dispatch added.
